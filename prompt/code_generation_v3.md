...
- **Simple description of dataset:** {data_info}
- **Previous Feedback (if any):** {feedback}
- **Current Iteration:** {iteration}
- **Statistical information of dataset** {final_data_report}. Select the columns you think are important for analysis.
...
**Generate two metadata files in JSON format:**
1.Image metadata JSON: Records all the metadata of the generated images, including file names, chart types, generation times, descriptions, etc.
2.Table metadata JSON: Records the file names, table types, data field descriptions, and other metadata of all CSV tables generated;
The JSON file should be in a standard format and encoded as UTF-8 to ensure readability and resolvability.
Metadata needs to be collected in real time (recorded when generating images/tables), and ultimately saved uniformly as the specified JSON file.
...
## 🧠 Improvements (if not the first iteration)

### 1. Error-Driven Code Improvement
- **Analyze previous errors** carefully and fix all issues
- If there was a traceback, **address the specific line mentioned**
- **Handle edge cases** that caused previous failures
- **Improve error handling** based on previous issues

### 2. Feedback Integration
- **Address all points** mentioned in the Quality Critic feedback
- **Prioritize critical issues** first (e.g., syntax errors, missing imports)
- **Implement suggestions** for code quality improvements
- **Enhance visualizations** if mentioned in feedback

### 3. Execution Readiness
- **Test your code mentally** for common issues:
  - Missing imports
  - Incorrect function calls
  - Undefined variables
  - File path issues
  - Memory limitations for large datasets
- **Ensure code runs end-to-end** without manual intervention
...
def plot_feature_importance(importance_dict, feature_names):
    ...
    plt.close()
```
**Make sure to use "data.csv" as the default file address in the generated code.**
...
-plot_metadata.json - All metadata for generated images (file name, type, generation time, size, description, etc.)
-table_Metadata.json - All metadata for generating CSV tables (file name, type, field description, description, etc.)
